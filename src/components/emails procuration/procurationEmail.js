/*
AKRAM GUELLOU = [
     // FOOT LOCKER lakram@hotmail.fr mandat 3// 
    JEANJEAN/JD SPORTS, mandat 1
]
ALEXANDRE BONNILLA = [
    // CARREFOUR RAMBOUILLET alexandre.bonilla@yahoo.fr mandat 10//
    CARREFOUR MERIGNAC, mandat 12
    CARREFOUR SUPPLY ERTECO, mandat 14
    CARREFOUR DUPARC, mandat 8
    CARREFOUR VAUX EN VELIN, mandat 1
]
BRIGITTE ASSAL = [
    // CLUB MED assalbrigitte92@gmail.com //
    DESMAZIERES-CHAUSSEXPO,
    CYRILLUS,
    AVIGNON TOURISME,
    MOVITEX,
]
CHARLES MIGUAN = [
    // SECURITE 3 migan.unsa@yahoo.fr //
    SMCP,
    BON MARCHE,
    AKTO,
]
ABDERRAZAK DJEFFEL = [
    // NETTOYAGE 2 unsa.proprete.nettoyage@gmail.com //
    GALERIES LAFAYETTES,
]
EL BACHIR MEKHLOUFI = [
    // NETTOYAGE 3 unsa.nettoyage@unsa.org //
    KUONI- TRAVEL LAB,
    AMPLITEL,
    JEAN MOULIN/JETLY,
    HAPPY CHIC,
]
FATIHA HIRAKI = [
    // SNSCS hfati@hotmail.fr //
    CARREFOUR MARKET,
]
FRANCK MARTINEZ = [
    // DECATHLON franck.martinez3@wanadoo.fr //
    J.A. DELMAS,
    KUONI GTS,
]
FREDDY HARDET = [
    // CARREFOUR SUPPLY CREPY freddy.hardet@orange.fr //
    RLS CE SNECMA,
    SERM
]
IMED BENAHMED = [
    // ZARA imed.benahmed@yahoo.fr //
    BERSHKA
]
KALILOU SIDIBE = [
    // SECURITE 2 sidibekalilou@yahoo.fr //
    ABERCROMBIE,
    SECURITE 7, // NOT AVAILABLE
    ASTEN SANTE,
    CASINO GD. CERCLE
]
KEMPF Jean Pierre = [
    // HCR unsa.hcr@unsa.org //
    CMS VACANCES,
    V R P
]
KHADIJA CHLAIT = [
    // NETTOYAGE 1 khadijachlait@hotmail.com //
    GFI INFORMATIQUE / INETUM,
    ITM LAI,
    LMNEXT FR,
]
MARYSE FUCHS = [
    // KIABI lagomuche@orange.fr //
    SONEPAR,
    PRIMAVISTA,
    D3T DISTRIBUTION,
    POLTRONESOFA,
]
MICHEL BRAQUET = [
    // BRAQUETMICHEL ?? missing ?? // 
    CARREFOUR SUPPLY COMBS,
    CONFORAMA,
    DESAUTEL,
    LYON PARC AUTO,
    OREFI  OUEST,
]
NADIA ZENAF = [
    // ETAM mouloud.nadia@neuf.fr //
    ALDI CAVAILLON,
    BOURSE DE L'IMMOBILIER,
    CELINE,
    THIRIET DISTRIBUTION,
]
SANDRINE POULIN = [
    // ADECCO unsa.adecco@gmail.com //
    EFR France/EG Retail France,
    GEZE France,
    QUALICONSULT,
    SUPERMARCHES MATCH
]
SANDRINE VERDIER = [
    // MANPOWER unsa.manpower@gmail.com //
    IKEA ENTREPOTS CHATRES,
    IKEA ENTREPOTS METZ,
    MC DONALD S France SERVICE,
    MINELLI,
]
SEBASTIEN FOURNIER = [
    // OFFICE DEPOT  fourseb13@gmail.com //
    AUCHAN NICE,
    BIOCOOP,
    CASINO ANTIBES SIESTA,
    LECLERC SAINTES, 
]
THIERRY PELMAR = [
    // SECURITE 1 pelmarthierry1@gmail.com //
    DENTSPLY SIRONA,
    RESMED,
    REXEL REGION SUD,
    VILMORIN JARDIN,
]
TOURE Ma Dembo = [
    // COMPASS compass@unsa.org // 
    MSC CROISIERES,
]
///////////////////// manque information ///////////////////////// 
ERIC LEPRINCE = [
    // ECOLAB PEST France unsa.epf@gmail.com //
    
]

// EMAIL A ENVOYER POUR LES VOTES PAR PROCURATION
[
    lakram@hotmail.fr,
    alexandre.bonilla@yahoo.fr,
    assalbrigitte92@gmail.com,
    migan.unsa@yahoo.fr,
    unsa.proprete.nettoyage@gmail.com,
    unsa.nettoyage@unsa.org,
    hfati@hotmail.fr,
    franck.martinez3@wanadoo.fr,
    freddy.hardet@orange.fr,
    imed.benahmed@yahoo.fr,
    sidibekalilou@yahoo.fr,
    unsa.hcr@unsa.org,
    khadijachlait@hotmail.com,
    lagomuche@orange.fr,
    mouloud.nadia@neuf.fr,
    unsa.adecco@gmail.com,
    unsa.manpower@gmail.com,
    fourseb13@gmail.com,
    pelmarthierry1@gmail.com,
    compass@unsa.org
    missing emails
    ABDELALI ELHAMOUTI, ECOLAB, MICHEL BRAQUET
]

// EMAIL A ENVOYER POUR LE VOTE / PERSONNE QUI VOTE POUR EUX MEME
[
    lecomtejeanmichel1970@gmail.com,
    bertrand.audoual@gmail.com,
    frpoignant@orange.fr,
    kristoy@msn.com,
    wiartfreddy62@gmail.com,
    catherineacho@live.fr,
    cse-lagoa@maisons.dumonde.com,
    unsa-maxicoffee-nord@outlook.fr,
    hassan.rammane@sfr.fr,
    ade.nacknouck@gmail.com,
    vanessaarveyres@gmail.com,
    tarik2113@hotmail.com,
    meherbouazza@gmail.com,
    m.essafir@gmail.com,
    alphajiddou@gmail.com,
    k.arbaoui@hotmail.fr,
    danielecursoli@gmail.com,
    lassananiakate1@yahoo.fr,
    caruanajodo@gmail.com,
    marchettiphilippe@sfr.fr,
    thierry.poirier@me.com,
    laubag@free.fr,
    christophe.franchini@orange.fr,
    stephane.cattier@ingka.ikea.com,
    bakim2@hotmail.fr,
    tissegouinepatricia@gmail.com,
    h.seghdau.unsa@gmail.com,
    pider33@hotmail.fr,
    thierry72b@sfr.fr,
    unsa.synergie@gmail.com,
]

*/
