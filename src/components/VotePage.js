	...
	useEffect(() => {
		switch (currentUser?.email) {
			...
			case 'assalbrigitte92@gmail.com':
				setSyndicatList(['CLUB MED', 'DESMAZIERES-CHAUSSEXPO', 'CYRILLUS', 'AVIGNON TOURISME', 'MOVITEX']);
				break;
			...
			case 'unsa.nettoyage@unsa.org':
				setSyndicatList(['NETTOYAGE 3', 'KUONI-TRAVEL LAB', 'AMPLITEL', 'JEAN MOULIN/JETLY', 'HAPPY CHIC']);
				break;
			...
			case 'freddy.hardet@orange.fr':
				setSyndicatList(['CARREFOUR SUPPLY CREPY', 'RLS CE SNECMA', 'SERM']);
				break;
			case 'imed.benahmed@yahoo.fr':
				setSyndicatList(['ZARAT', 'BERSHKA']);
				break;
			case 'sidibekalilou@yahoo.fr':
				setSyndicatList(['SECURITE 2', 'ABERCROMBIE', 'SECURITE 7', 'ASTEN SANTE', 'CASINO GD']);
				break;
			case 'unsa.hcr@unsa.org':
				setSyndicatList(['HCR', 'CMS VACANCES', 'V R P']);
				break;
			case 'khadijachlait@hotmail.com':
				setSyndicatList(['NETTOYAGE 1', 'GFI INFORMATIQUE', 'ITM LAI', 'LMNEXT FR']);
				break;
			case 'lagomuche@orange.fr':
				setSyndicatList(['KIABI', 'SONEPAR', 'PRIMAVISTA', 'D3T DISTRIBUTION', 'POLTRONESOFA']);
				break;
			case 'mouloud.nadia@neuf.fr':
				setSyndicatList(['ETAM', 'ALDI CAVAILLON', "BOURSE DE L'IMMOBILIER", 'CELINE', 'THIRIET DISTRIBUTION']);
				break;
			case 'unsa.adecco@gmail.com':
				setSyndicatList(['ADECCO', 'EFR France/EG Retail France', 'GEZE France', 'QUALICONSULT', 'SUPERMARCHES MATCH']);
				break;
			case 'unsa.manpower@gmail.com':
				setSyndicatList(['MANPOWER', 'IKEA ENTREPOTS CHATRES', 'IKEA ENTREPOTS METZ', 'MC DONALD S France SERVICE', 'MINELLI']);
				break;
			case 'fourseb13@gmail.com':
				setSyndicatList(['OFFICE DEPOT', 'AUCHAN NICE', 'BIOCOOP', 'CASINO ANTIBES SIESTA', 'LECLERC SAINTES']);
				break;
			case 'pelmarthierry1@gmail.com':
				setSyndicatList(['SECURITE 1', 'DENTSPLY SIRONA', 'RESMED', 'REXEL REGION SUD', 'VILMORIN JARDIN']);
				break;
