...
const VotePage = () => {
	...
	const { users, procurationUsers, syndicatList, setSyndicatList, query, setQuery, resultBureau } = useGlobalContext();
		...
		<div className='flex flex-col py-8 h-1/2'>
			{procurationUsers && (
				<div className='bg-white flex flex-col justify-between pl-3 pr-3 w-11/12 m-auto shadow-lg rounded uppercase py-8 mb-8'>
					<h2 className='pb-5 font-medium self-center text-xl sm:text-2xl uppercase text-gray-500'>Page de Vote</h2>
			...
			<div className={`${procurationUsers ? 'hidden' : 'w-11/12 m-auto shadow-lg bg-white rounded-md'}`}>
				<div className={`${voting ? 'hidden' : 'flex flex-col py-8 px-5'}`}>
					<h2 className='pb-10 font-medium self-center text-xl sm:text-2xl uppercase text-gray-500'>Bienvenue sur la page de vote de L'UNSA</h2>
