...
const VotePage = () => {
	...
	const { users, procurationUsers, syndicatList, setSyndicatList } = useGlobalContext();
	...
	useEffect(() => {
		switch (currentUser.email) {
			...
			case 'alexandre.bonilla@yahoo.fr':
				setSyndicatList(['CARREFOUR MERIGNAC', 'CARREFOUR SUPPLY ERTECO', 'CARREFOUR DUPARC', 'CARREFOUR VAUX EN VELIN']);
				break;
			case 'assalbrigitte92@gmail.com':
				setSyndicatList(['DESMAZIERES-CHAUSSEXPO', 'CYRILLUS', 'AVIGNON TOURISME', 'MOVITEX']);
				break;
			...
			case 'unsa.nettoyage@unsa.org':
				setSyndicatList(['KUONI-TRAVEL LAB', 'AMPLITEL', 'JEAN MOULIN/JETLY', 'HAPPY CHIC']);
				break;
			...
			case 'sidibekalilou@yahoo.fr':
				setSyndicatList(['ABERCROMBIE', 'SECURITE 7', 'ASTEN SANTE', 'CASINO GD']);
				break;
			...
			case 'lagomuche@orange.fr':
				setSyndicatList(['SONEPAR', 'PRIMAVISTA', 'D3T DISTRIBUTION', 'POLTRONESOFA']);
				break;
			case 'mouloud.nadia@neuf.fr':
				setSyndicatList(['ALDI CAVAILLON', "BOURSE DE L'IMMOBILIER", 'CELINE', 'THIRIET DISTRIBUTION']);
				break;
			case 'unsa.adecco@gmail.com':
				setSyndicatList(['EFR France/EG Retail France', 'GEZE France', 'QUALICONSULT', 'SUPERMARCHES MATCH']);
				break;
			case 'unsa.manpower@gmail.com':
				setSyndicatList(['IKEA ENTREPOTS CHATRES', 'IKEA ENTREPOTS METZ', 'MC DONALD S France SERVICE', 'MINELLI']);
				break;
			case 'fourseb13@gmail.com':
				setSyndicatList(['AUCHAN NICE', 'BIOCOOP', 'CASINO ANTIBES SIESTA', 'LECLERC SAINTES']);
				break;
			case 'pelmarthierry1@gmail.com':
				setSyndicatList(['DENTSPLY SIRONA', 'RESMED', 'REXEL REGION SUD', 'VILMORIN JARDIN']);
				break;
		...
		<>
			<div className='flex flex-col py-8 h-1/2'>
				<div className='bg-white flex flex-col justify-between pl-3 pr-3 w-11/12 m-auto shadow-lg rounded uppercase py-8 mb-8'>
					...
					{procurationUsers && (
						<div>
							<span className='text-sm font-medium m-auto mb-3 mt-5'>Vous avez une procuration pour</span>
				...
				<div className='w-11/12 m-auto shadow-lg bg-white rounded-md'>
					<div>
						<div className={`${voting ? 'hidden' : 'flex flex-col py-8 px-5'}`}>
							<h2 className='text-center pb-10 font-medium self-center text-xl sm:text-2xl uppercase text-gray-500'>Bienvenue sur la plateforme de vote de L'UNSA FCS</h2>
							<button onClick={() => setVoting(!voting)} className='m-auto w-auto py-2 px-4 border border-transparent shadow-sm text-xs sm:text-sm  uppercase rounded-md text-white transition duration-150 ease-in bg-blue-600 hover:bg-blue-700 focus:outline-none'>
								Commencer
							</button>
						</div>
					</div>
