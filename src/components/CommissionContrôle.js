...
const CommissionControle1 = ({ mandat, syndicat, email }) => {
	...
	const onSubmit = async () => {
		if (watchCheckBox && checkedCount === 3) {
			await addDoc(collection(db, 'results1'), {
				...
				.then(() => {
					alert('Votre vote pour la commission de contrôle a été prise en compte');
				})
				.catch((error) => {
	...
	const voted = resultControle.find(({ data: { email, syndicat } }) => email === `${!procurationUsers && currentUser.email}` || syndicat === `${procurationUsers && query}`);
				...
				<form onSubmit={handleSubmit(onSubmit)}>
					<div className='w-11/12 m-auto shadow-lg bg-white rounded-md'>
						<div className='flex flex-col py-8 px-5'>
							{!procurationUsers && <h2 className='pb-5 font-medium self-center text-xl sm:text-2xl uppercase text-gray-500'>Page de Vote</h2>}
							<div className='flex justify-between text-base font-medium text-gray-900 uppercase sm:p-6 py-5 px-2'>
								<legend className='text-xs sm:text-sm lg:text-lg font-bold text-gray-900'>La commission de contrôle</legend>
							...
							<div className='mt-5 ml-7 space-y-4'>
								...
								<div className='flex items-start'>
									<div className='flex items-center h-5'>
										<input name='a' {...register('a')} type='checkbox' onClick={HandleA} value={a} className='focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded' /> <span className='ml-3 text-xs sm:text-sm lg:text-base font-medium text-gray-700'>OPTION 1</span>
									</div>
								</div>
								<div className='flex items-start'>
									<div className='flex items-center h-5'>
										<input name='b' {...register('b')} type='checkbox' onClick={HandleB} value={b} className='focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded' /> <span className='ml-3 text-xs sm:text-sm lg:text-base font-medium text-gray-700'>OPTION 2</span>
									</div>
								</div>
								<div className='flex items-start'>
									<div className='flex items-center h-5'>
										<input name='c' {...register('c')} type='checkbox' onClick={HandleC} value={c} className='focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded' /> <span className='ml-3 text-xs sm:text-sm lg:text-base font-medium text-gray-700'>OPTION 3</span>
									</div>
								</div>
								<div className='flex items-start'>
									<div className='flex items-center h-5'>
										<input name='d' {...register('d')} type='checkbox' onClick={HandleD} value={d} className='focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded' /> <span className='ml-3 text-xs sm:text-sm lg:text-base font-medium text-gray-700'>OPTION 4</span>
									</div>
								</div>
								<div className='flex items-start'>
									<div className='flex items-center h-5'>
										<input name='e' {...register('e')} type='checkbox' onClick={HandleE} value={e} className='focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded' /> <span className='ml-3 text-xs sm:text-sm lg:text-base font-medium text-gray-700'>OPTION 5</span>
									</div>
								</div>
							</div>
						</div>
